...
import * as render from './render1.js';
...
//   let list = [];
//   for (const [id, title, body] of db.query(sql, args)) {
//     list.push({ id, title, body });
//   }
//   return list;
// }
...
async function userList(ctx) {
  ...
  const users = [...new Set(db.query("SELECT DISTINCT user FROM posts").map(row => row[0]))];
